// se agrego status bomba y se esta agregando programador de riego
...
int pin_bomba_riego = PIN_BOMBA_RIEGO;
...
//  while(!Serial);
  ...
  File myFile = SD.open("riego.csv");
  String lines_riego[246];
  String line = "";
  int count_line = 0;
  if (myFile) {
    Serial.println("riego.csv:");
    // read from the file until there's nothing else in it:
    while (myFile.available()) {
      data_in_ = myFile.read();
      Serial.write(data_in_);
      line += data_in_;
      if(data_in_ == '\n'){
        lines_riego[count_line] = line;
        count_line++;
        Serial.print("nueva linea!!!!! :\t");
        Serial.println(line);
        line = "";
      }
    }
    ...
    myFile.close();
    for(int i=0; i<246; i++){
      Serial.print(lines_riego[i]);  
    }
    // parse riego in array
    for(int i=1; i<246; i++){
      token = (char *)lines_riego[i].c_str();
      token = strtok(token, ",");
      int count_cols = 0;
      while(token != NULL){
        if(count_cols < 7){
          prog_riego[i][count_cols] = atoi(token);
          count_cols++;
        }
        token = strtok(NULL, ",");
      } 
...
void loop()
{
  ...
  if(GPIO_3 == LOW){ // si hay agua
    ...
    else if(GPIO_1 == LOW){ // si se enciende auto_internet
      auto_internet = true;
      if( (GPIO_2 == LOW) && (hora_punta == false) ){ // si pide agua la casa patronal y no es hora punta
        bomba_on = true;
        casa_patronal = true;
      }
      else {
...
*/
        ...
        if( (auto_internet == true) /*&& (enable_var_frec == true)*/ ){
          ...
          if(strstr(bufferAnswer, "\r\n1.0\r\n")){
            if( (bomba_on == false) && (enable_var_frec == true) && (hora_punta == false)){
              bomba_on = true;
              //var_frec.writeSingleRegister(0x08, 0x02); // dar partida
            }
            Serial1.println("Remote Bomba ON");
          }
          else{
            if(bomba_on == true){
          ...
          if( (freq >=30) && (freq <= 55) ){
            ...
            if( freq_prev != freq__){
              ...
              var_frec.writeSingleRegister(REG_RUN_FREQ, freq__); // setear frecuencia
              freq_prev = freq__;
            }
          ...
          if( (bomba_on == true) && (enable_var_frec == false) ){
            bomba_on = false;
            var_frec.writeSingleRegister(0x08, 0x00); // parar 
          }  
        }
//      }
        String caudal__ = "";
        caudal__ += caudal;
        int largo_del_json = 210 + caudal__.length();
        largo_del_json += (enable_var_frec == true)?(3):(1);
        Serial1.println(largo_del_json);
        tmp = "POST /api/v1.6/collections/values/?token=BBFF-IH5I4OcjNkdFY6IAcKjnBhbdEThV8j HTTP/1.1\r\n";
        tmp += "Host: things.ubidots.com\r\n";
        ...
        tmp += ": ";
        tmp += '"'; // 17
        tmp += "5ada3c57642ab6080b6e4d29"; // 24
        tmp += '"';
        tmp += ", ";
        tmp += '"';
        tmp += "value"; // 150
        ...
        tmp += ": ";
        tmp += '"'; // 170
        tmp += "5adb80c2642ab6033af94cd4"; // 194
        tmp += '"';
        tmp += ", ";
        tmp += '"';
        tmp += "value";
  ...
  if( (bomba_on == true) && (hora_punta == false) && (enable_var_frec == true) ){
    var_frec.writeSingleRegister(0x08, 0x02); // dar partida
...
int init_programador_riego(void){
  pinMode(pin_bomba, OUTPUT);
...
int programador_riego(void){
  current_time = Teensy3Clock.get();
  Serial.println(current_time);
  long modulo = current_time%604800;
  int dia_de_la_semana = (modulo >= 0 && modulo < 86400)?(4):
                         (modulo >= 86400 && modulo < 172800)?(5):
                         (modulo >= 172800 && modulo < 259200)?(6):
                         (modulo >= 259200 && modulo < 345600)?(7):
                         (modulo >= 345600 && modulo < 432000)?(1):
                         (modulo >= 432000 && modulo < 518400)?(2):
                         (modulo >= 518400 && modulo < 604800)?(3):(0);
  hora = (((current_time%31536000)%2592000)%86400)/3600;
  minuto = ((((current_time%31536000)%2592000)%86400)%3600)/60;
  segundo = ((((current_time%31536000)%2592000)%86400)%3600)%60;

  if(flag_salida_on == true){
    for(int i=0; i<7; i++){ // por cada salida verifico su hora de inicio
      if( (hora == prog_riego[(programa_actual - 1)*7*7 + (dia_de_la_semana - 1)*7 + i][5]) ){
        if(minuto == prog_riego[(programa_actual - 1)*7*7 + (dia_de_la_semana - 1)*7 + i][6]){
          switch(i){
            case 0 :
              digitalWrite(pin_bomba_riego, LOW);
              digitalWrite(pin_salida_1, LOW);
              flag_salida_on = false;
              break;
            case 1:
              digitalWrite(pin_bomba_riego, LOW);
            ...
            case 6:
              ...
              break;
            default :
              Serial.println("error!: index fault in riego.");
              break;
          }  
        }
      }
  ...
  if(flag_salida_on == false){
    for(int i=0; i<7; i++){ // por cada salida verifico su hora de inicio
      if( (hora == prog_riego[(programa_actual - 1)*7*7 + (dia_de_la_semana - 1)*7 + i][3]) ){
        if(minuto == prog_riego[(programa_actual - 1)*7*7 + (dia_de_la_semana - 1)*7 + i][4]){
          switch(i){
            case 0 :
